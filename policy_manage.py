...
Subcommands:
  ...
  assign — add or remove a peer from the user's servers group (GET/merge/PUT /api/groups).

Environment: NETBIRD_API_BASE, NETBIRD_TOKEN (or --token). Loads ``.env`` from the current working directory.

Run: uv run python policy_manage.py ensure-user-groups --email user@example.com --dry-run
"""
...
def _emails_from_args(
    ...
    if file_path:
        try:
            ...
            return [], "internal error: could not import user_manage.load_rows"
        try:
...
def run_ensure(args: argparse.Namespace) -> int:
    ...
    for i, email in enumerate(emails):
        res = ensure_user_groups_and_policy(
            ...
            update_user_auto_groups=args.update_user_auto_groups,
...
def run(argv: list[str] | None = None) -> int:
    ...
    p_assign.add_argument("--email", required=True, help="Target user email")
    p_assign.add_argument("--peer-id", default="", help="NetBird peer id")
    p_assign.add_argument("--peer-name", default="", help="Peer hostname (exact match)")
